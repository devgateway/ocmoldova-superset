...
-- Completed on 2023-06-13 12:52:40 UTC
...
CREATE OR REPLACE FUNCTION extra.not_null_count(_table_name text, _column_name text)
  RETURNS setof bigint AS $$
BEGIN
RETURN QUERY EXECUTE
 'SELECT count(*) FROM ' || $1 || ' WHERE "'  || $2 ||  '" is not null';
END;
$$ LANGUAGE plpgsql


CREATE OR REPLACE FUNCTION extra.not_null_count_distinct(_table_name text, _column_name text)
  RETURNS setof bigint AS $$
BEGIN
RETURN QUERY EXECUTE
 'SELECT count(distinct "' || $2 || '") FROM ' || $1 || ' WHERE "'  || $2 ||  '" is not null';
END;
$$ LANGUAGE plpgsql


-- FUNCTION: extra.table_stats(text)

-- DROP FUNCTION IF EXISTS extra.table_stats(text);

CREATE OR REPLACE FUNCTION extra.table_stats(
	schema_name text)
    RETURNS TABLE(table_name information_schema.sql_identifier, attname name, cnt bigint) 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
RETURN QUERY
select t.table_name, a.attname, extra.not_null_count(schema_name ||'.' || t.table_name, a.attname) as cnt from pg_attribute a, 
information_Schema.tables t, pg_class c where t.table_schema= schema_name
and c.relname=t.table_name AND a.atttypid != 'pg_catalog.name'::regtype AND a.attnum>0 AND c.oid=a.attrelid
AND not a.attisdropped order by t.table_name, cnt desc;
END;
$BODY$;

ALTER FUNCTION extra.table_stats(text)
    OWNER TO superset;
